...
class GUIManager {
  ...
  int buttonColor = color(50,150,150);
  PFont buttonFont;
...
public PImage flipimage(PImage toflip) {
  ...
  for (int i=0; i < width; i++) {
    for (int j=0; j < toflip.height; j++) {
      ...
      mask.set(i, j, PApplet.parseInt(alpha(toflip.get(toflip.width-i-1, j))));
    }
  }
  //PImage mask = output;
  mask.filter(GRAY);
  mask.filter(THRESHOLD, 0.1f);
  //mask.filter(INVERT);
  output.mask(mask);
  //output = mask;
...
public void menuSetup() {
  ...
  GUI.addButton("settingsButton", "Settings", 0.3f, 0.6f, 300, 50);
}
public void menu() {
  imageMode(CENTER);
